    ...
    WinJS.xhr({ url: url, responseType: "JSON" }).then(
        function complete(result) {
            if (result.status === 200) {
                ...
                //将对象传递给下一步进行处理
                AnalyseHDFInfo(obj);
                totalCount = myRESTUrlArray.length;
                DownloadIMG();
            }
        },
        function error(
    ...
    for (var i = 0; i < LevelCount; i++) {
        ...
        for (var m = 0; m < rowLimitNowLevel; m++) {
            for (var n = 0; n < colLimitNowLevel; n++) {
                ...
                //中转
                //var objHere = {url:"http://192.168.83.185/r2rest/Handler.ashx?method=GetSingleHDF&gisip=192.168.83.185&gisport=6163&hdfname=fjMap&l=" + levelNow + "&r=" + m + "&c=" + n,l:levelNow,r:m,c:n};
                myRESTUrlArray.push(objHere);
            }
        }
    }
}
...
function DownloadIMG() {
    if (nowCount < totalCount) {
        var url = myRESTUrlArray[nowCount].url;
        WinJS.xhr({ url: url, responseType: "blob" }).then(
           function complete(result) {
               if (result.status === 200) {
                   WriteIMGToFolder(result.response, myRESTUrlArray[nowCount].l, myRESTUrlArray[nowCount].r, myRESTUrlArray[nowCount].c);
                   nowCount++;
                   DownloadIMG();
                   //下载进度条
                   var progressObj = $("#myprogress");
                   var oldValue = progressObj.attr("value");
                   progressObj.attr("value", oldValue + parseFloat(100 / totalCount));
...
//方法：将下载到的图片写入指定文件夹
function WriteIMGToFolder(blob,l,r,c) {
    Windows.Storage.ApplicationData.current.localFolder.createFolderAsync("map", Windows.Storage.CreationCollisionOption.openIfExists).then(function (folder) {
...
//方法：清空当前存在的地图文件
function DeleteIMGFolder() {
    Windows.Storage.ApplicationData.current.localFolder.getFolderAsync("map").then(
